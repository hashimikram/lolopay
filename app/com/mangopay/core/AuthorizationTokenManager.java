...
public final class AuthorizationTokenManager extends ApiBase {
   ...
   *
   * <p>In the very first call, this method creates a new token before returning. If currently
  ...
  /**
   * Stores authorization token passed as an argument in the underlying storage strategy
   * implementation.
   *
   ...
   *
   * <p>By default, the <code>DefaultStorageStrategy</code> instance is used. There is no need to
   * explicitly call this method until some more complex storage implementation is needed.
   *
  ...
  private String getEnvKey() {
    ...
    String input =
        root.getConfig().getBaseUrl()
            + root.getConfig().getClientId()
            + root.getConfig().getClientPassword();
    String md5 = "";
