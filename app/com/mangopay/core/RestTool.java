...
public class RestTool {
  ...
  public void addRequestHttpHeader(Map<String, String> httpHeader) {
    ...
    if (this.requestHttpHeaders == null) this.requestHttpHeaders = new HashMap<>();
   ...
   *
   * <p>This generic method handles calls targeting single <code>Dto</code> instances. In order to
   * process collections of objects, use <code>requestList</code> method instead.
   *
   ...
   * @param entity Instance of Dto class that is going to be sent in case of PUTting or POSTing.
   ...
   */
  public <T extends Dto> T request(
      Class<T> classOfT, String idempotencyKey, String urlMethod, String requestType)
      throws Exception {
   ...
   * Makes a call to the MangoPay API.
   *
   * <p>This generic method handles calls targeting collections of <code>Dto</code> instances. In
   * order to process single objects, use <code>request</code> method instead.
   *
   * @param <T> Type on behalf of which the request is being called.
   * @param classOfT Type on behalf of which the request is being called.
  ...
  public <T extends Dto> List<T> requestList(
    ...
    List<T> responseResult =
        this.doRequestList(classOfT, classOfTItem, urlMethod, pagination, additionalUrlParams);
   ...
   */
  public <T extends Dto> List<T> requestList(
      Class<T[]> classOfT, Class<T> classOfTItem, String urlMethod, String requestType)
      throws Exception {
  ...
  public <T extends Dto> List<T> requestList(
    ...
    return requestList(
        classOfT, classOfTItem, urlMethod, requestType, requestData, pagination, null);
  }

  private <T extends Dto> T doRequest(
      Class<T> classOfT, String idempotencyKey, String urlMethod, Pagination pagination, T entity)
      throws Exception {
    ...
    try {
      ...
      System.out.println("FullUrl: " + urlTool.getFullUrl(restUrl));
      if (this.debugMode) {
        logger.info("FullUrl: {}", urlTool.getFullUrl(restUrl));
      }

      /*
       * FOR WEB DEBUG PURPOSES SocketAddress addr = new InetSocketAddress("localhost", 8888); Proxy proxy = new Proxy(Proxy.Type.HTTP, addr); connection = (HttpURLConnection)url.openConnection(proxy);
       */
      ...
      System.out.println("RequestType: " + this.requestType);
      if (this.debugMode) logger.info("RequestType: {}", this.requestType);
      ...
      if (this.requestData != null || entity != null) {
        ...
        System.out.println("RequestData: " + this.requestData);
        if (this.debugMode) {
        ...
        // CUSTOM don't send post wuth null body
      ...
      if (this.responseCode == 200) {
        ...
        response =
            castResponseToEntity(
                classOfT, new JsonParser().parse(responseString).getAsJsonObject());
  ...
  private List<RateLimit> initRateLimits() {
    ...
    return Arrays.asList(
        new RateLimit(15), new RateLimit(30), new RateLimit(60), new RateLimit(24 * 60));
  }
  ...
  private <T> Boolean canReadSubRequestData(Class<T> classOfT, String fieldName) {
    ...
    if (classOfT.getName().equals(PayOut.class.getName())
        && fieldName.equals("MeanOfPaymentDetails")) {
      return true;
    }
  ...
  private <T> T castResponseToEntity(
      Class<T> classOfT, JsonObject response, boolean asDependentObject) throws Exception {
    ...
    try {
        ...
        for (Entry<String, JsonElement> entry : response.entrySet()) {
          ...
          if (entry.getKey().equals("PersonType")) {
            ...
            if (userType.equals(PersonType.NATURAL.toString())) {
              ...
              throw new Exception(
                  String.format("Unknown type of user: %s", entry.getValue().getAsString()));
            }
          }
        }
      } else {
      ...
      Map<String, Map<String, Map<String, Class<?>>>> dependentObjects =
          ((Dto) result).getDependentObjects();
            ...
            if (entry.getKey().equals(name)) {
              ...
              if (allowedTypes.containsKey(paymentTypeDef)) {
                ...
                for (Entry<String, Class<?>> e : targetObjectsDef.entrySet()) {
                  ...
                  if (isList) {
                    targetField.set(
                        result, Arrays.asList(castResponseToEntity(e.getValue(), response, true)));
                  } else {
          ...
          if (entry.getKey().equals(name)) {
            ...
            if (subObjects.containsKey(name)) {
              if (entry.getValue() instanceof JsonNull) {
                ...
                f.set(
                    result, castResponseToEntity(f.getType(), entry.getValue().getAsJsonObject()));
              }
              break;
            }
            ...
            if (debugMode) {
              logger.info(
                  "Recognized field: {}",
                  String.format("[%s] %s%s", name, fieldTypeName, fieldIsArray ? "[]" : ""));
            }
              ...
              if (entry.getValue().isJsonArray()) {
                ...
                while (i.hasNext()) {
                  ...
                  if (genericTypeClass.getName().equals(String.class.getName())) {
                    ...
                    addMethod.invoke(
                        o, castResponseToEntity(genericTypeClass, e.getAsJsonObject()));
                  }
                }
                f.set(result, o);
              }
  ...
  private <T extends Dto> List<T> doRequestList(
      Class<T[]> classOfT, Class<T> classOfTItem, String urlMethod, Pagination pagination)
      throws Exception {
  ...
  private <T extends Dto> List<T> doRequestList(
    ...
    try {
      UrlTool urlTool = new UrlTool(root);
      String restUrl =
          urlTool.getRestUrl(urlMethod, this.authRequired, pagination, additionalUrlParams);

      URL url = new URL(urlTool.getFullUrl(restUrl));

      if (this.debugMode) logger.info("FullUrl: {}", urlTool.getFullUrl(restUrl));
  ...
  private Map<String, String> getHttpHeaders(String restUrl) throws Exception {
    ...
    if (this.requestHttpHeaders != null) return this.requestHttpHeaders;
