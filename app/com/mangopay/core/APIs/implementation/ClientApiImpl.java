...
public class ClientApiImpl extends ApiBase implements ClientApi {
   ...
   */
  public ClientApiImpl(MangoPayApi root) {
    super(root);
  }

  @Override
  public List<KycDocument> getKycDocuments(
      Pagination pagination, FilterKycDocuments filter, Sorting sort) throws Exception {
  ...
  public void uploadLogo(byte[] binaryData) throws Exception {
    String fileContent =
        new String(org.apache.commons.codec.binary.Base64.encodeBase64(binaryData));
  ...
  public List<Wallet> getWallets(FundsType fundsType, Pagination pagination) throws Exception {
    ...
    switch (fundsType) {
      ...
      case FEES:
        return this.getList(
            Wallet[].class, Wallet.class, "client_get_wallets_fees", pagination, null, null, null);
      case CREDIT:
        return this.getList(
            Wallet[].class,
            Wallet.class,
    ...
    switch (fundsType) {
      case DEFAULT:
        return this.getObject(
            Wallet.class, "client_get_wallets_default_with_currency", currency.toString());
      case FEES:
        return this.getObject(
            Wallet.class, "client_get_wallets_fees_with_currency", currency.toString());
      case CREDIT:
        return this.getObject(
            Wallet.class, "client_get_wallets_credit_with_currency", currency.toString());
    }
  ...
  public List<Transaction> getTransactions(
      Pagination pagination, FilterTransactions filter, Sorting sort) throws Exception {
    if (filter == null) filter = new FilterTransactions();
  ...
  @Override
  public PayIn createBankWireDirect(String idempotencyKey, ClientBankWireDirect bankWireDirect)
      throws Exception {
    PayIn payIn = new PayIn();
    payIn.setCreditedWalletId(bankWireDirect.getCreditedWalletId());
    payIn.setPaymentDetails(new PayInPaymentDetailsBankWire());
    ((PayInPaymentDetailsBankWire) payIn.getPaymentDetails())
        .setDeclaredDebitedFunds(bankWireDirect.getDeclaredDebitedFunds());
