...
public class PayInApiImpl extends ApiBase implements PayInApi {
  ...
  public PayIn create(String idempotencyKey, PayIn payIn) throws Exception {
    ...
    return this.createObject(
        PayIn.class,
        idempotencyKey,
        String.format("payins_%s-%s_create", paymentKey, executionKey),
        payIn);
  }
  ...
  @Override
  public Refund createRefund(String idempotencyKey, String payInId, Refund refund)
      throws Exception {
  ...
  @Override
  public List<Refund> getRefunds(String payInId, Pagination pagination, Sorting sorting)
      throws Exception {
    return this.getList(
        Refund[].class, Refund.class, "payin_get_refunds", pagination, payInId, sorting);
  }
  ...
  private String getPaymentKey(PayIn payIn) throws Exception {
    ...
    String className =
        payIn.getPaymentDetails().getClass().getSimpleName().replace("PayInPaymentDetails", "");
  ...
  private String getExecutionKey(PayIn payIn) throws Exception {
    ...
    String className =
        payIn.getExecutionDetails().getClass().getSimpleName().replace("PayInExecutionDetails", "");
    return className.toLowerCase();
  }
}
